...
def main():
    match sys.argv[1:]:
        ...
        case ["write-tree"]:
            parent = Path(".")
            def to_Entry(p:Path, exclude_git:bool=False) -> Tuple[str,str,str]:
                ...
                if p.is_dir():
                    ...
                    for child in p.iterdir():
                        if exclude_git and child.name == ".git":
                            continue
                        entries.append(to_Entry(child))
                    s_entries = sorted(entries, key=itemgetter(1))
                    b_entries = b"".join(
                        mode.encode() + b" " + name.encode() + b"\0" + bytes.fromhex(hex) for (mode,name,hex) in s_entries
                    )
            ...
            (_,_,hash) = to_Entry(Path(".").absolute(), True)
            print(hash)
        case ["commit-tree", tree_sha, "-p", commit_sha, "-m", message]:
            contents = b"".join(
                [
                    b"tree %b\n" % tree_sha.encode(),
                    b"parent %b\n" % commit_sha.encode(),
                    b"author yashk <yashwanthkothakota@gmail.com> 1714599041 -0600\n",
                    b"committer yashk <yashwanthkothakota@gmail.com> 1714599041 -0600\n\n",
                    message.encode(),
                    b"\n"
                ]
            )
